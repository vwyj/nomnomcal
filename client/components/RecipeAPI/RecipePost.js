...
const RecipePost = ({ posts, myPostScreen }) => {
    ...
    const handleDeletePost = async (id) => {
        ...
        {
            setLoading(true);
            const {data} =await axios.delete(`http://192.168.18.34:5000/api/v1/post/delete-post/${id}`);
            //const {data} =await axios.delete(`http://172.20.10.2:5000/api/v1/post/delete-post/${id}`);
            setLoading(false);
            alert(data?.message);
