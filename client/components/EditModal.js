...
const EditModal = ({ modalVisible, setModalVisible, post }) => {
    ...
    const updatePostHandler = async (id) => {
        try 
        {
            const {data} = await axios.put(`http://192.168.18.34:5000/api/v1/post/update-post/${id}`, {title, ingredients, instructions, calorie});
            //const {data} = await axios.put(`http://172.20.10.2:5000/api/v1/post/update-post/${id}`, {title, ingredients, instructions, calorie});
            alert(data?.message)
